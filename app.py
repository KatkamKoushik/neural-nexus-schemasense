...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
st.set_page_config(
    page_title="SchemaSense AI ΓÇô Intelligent Data Dictionary Agent",
    page_icon="≡ƒºá",
    layout="wide",
...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# CUSTOM CSS ΓÇô glassmorphism, gradients, animations
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
st.markdown("""
<style>
/* ---------- Google Font ---------- */
...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# FEATURE 1 ΓÇô ROBUST API KEY FALLBACK
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            if "429" in err_str or "quota" in err_str.lower() or "rate" in err_str.lower():
                ...
                st.toast(f"ΓÜá∩╕Å Key #{attempt + 1} quota hit ΓÇô switching to next keyΓÇª", icon="≡ƒöä")
            else:
...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# FEATURE 2 ΓÇô SQLITE MULTI-DATASET ENGINE
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def load_csvs_into_sqlite(files) -> tuple[sqlite3.Connection | None, dict]:
    ...
    for f in files:
        # Sanitize filename ΓåÆ table name
...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# FEATURE 3 ΓÇô SQL EXTRACTION HELPER
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {
            "role": "assistant",
            "content": "≡ƒæï **Data connection established!** I'm your AI Data Steward. Ask me to describe tables, write SQL queries, or analyze data quality.",
        }
    ]
...
with st.sidebar:
    st.markdown(
        ...
        '<span style="font-size:2.2rem">≡ƒºá</span>'
    ...
    # ΓöÇΓöÇ AI Engine ΓöÇΓöÇ
    st.subheader("ΓÜÖ∩╕Å AI Engine")
    _MODEL_OPTIONS = [
        "gemini-2.0-flash",
    ...
    _MODEL_LABELS = {
        ...
        "gemini-2.0-flash-lite": "Gemini 2.0 Flash Lite (Fastest ┬╖ Low Quota)",
        "gemini-1.5-flash":      "Gemini 1.5 Flash (Stable ┬╖ Balanced)",
    ...
    # ΓöÇΓöÇ Data Source ΓöÇΓöÇ
    with st.sidebar.container():
        st.subheader("≡ƒöî Data Source")
        ...
        if data_source == "Upload Custom Dataset":
            ...
            if uploaded_files:
                ...
                if len(uploaded_files) > 1:
                    st.caption(f"≡ƒôª {len(uploaded_files)} tables loaded into SQLite")
                    for tname, cols in schema_info.items():
                        with st.expander(f"≡ƒôï {tname} ({len(cols)} cols)"):
            ...
            # ΓöÇΓöÇ Live PostgreSQL connection status ΓöÇΓöÇ
            if pg_conn is not None:
                st.success("≡ƒƒó **Connected** ┬╖ Olist PostgreSQL ┬╖ Cloud SQL", icon=None)
                st.caption("Live database ┬╖ 9 tables ┬╖ queries cached 10 min")
            else:
                st.error("≡ƒö┤ **DB Unavailable** ┬╖ Could not connect to PostgreSQL")
                if pg_conn_error:
        ...
        with st.sidebar.container():
            st.sidebar.divider()
            st.sidebar.file_uploader("≡ƒôé Upload Business Rules (PDF)", type=['pdf'], key='biz_rules')
        ...
        if st.session_state.get('biz_rules') is not None:
            if _PYPDF2_AVAILABLE:
                try:
                    ...
                    if business_context:
                        st.success(f"Γ£à PDF ingested ┬╖ {len(pdf_reader.pages)} page(s) ┬╖ {len(business_context):,} chars")
                    else:
                        st.warning("ΓÜá∩╕Å PDF uploaded but no text could be extracted (may be image-based).")
                except Exception as _pdf_err:
                    business_context = "PDF attached, but text extraction skipped."
                    st.warning("ΓÜá∩╕Å PDF attached, but text extraction skipped.")
            else:
                st.error("Γ¥î PyPDF2 not installed. Add `PyPDF2` to requirements.txt and redeploy.")
    
    st.divider()

    # ΓöÇΓöÇ Actions ΓöÇΓöÇ
    with st.sidebar.container():
        st.subheader("ΓÜí Actions")
        ...
        st.download_button(
            label="≡ƒôÑ Download Data Dictionary",
        ...
        if st.button("≡ƒùæ∩╕Å Clear Conversation", use_container_width=True):
            st.session_state.messages = [
                {
                    "role": "assistant",
                    "content": "≡ƒæï Conversation cleared. How can I help?",
                }
            ]
            st.session_state.last_sql = None
            st.rerun()
...
Available tables:
  ΓÇó olist_orders_dataset (order_id, customer_id, order_status, order_purchase_timestamp, order_delivered_customer_date, ΓÇª)
  ΓÇó olist_order_items_dataset (order_id, order_item_id, product_id, seller_id, price, freight_value, ΓÇª)
  ΓÇó olist_products_dataset (product_id, product_category_name, product_weight_g, product_length_cm, ΓÇª)
  ΓÇó olist_customers_dataset (customer_id, customer_unique_id, customer_zip_code_prefix, customer_city, customer_state)
  ΓÇó olist_sellers_dataset (seller_id, seller_zip_code_prefix, seller_city, seller_state)
  ΓÇó olist_order_payments_dataset (order_id, payment_sequential, payment_type, payment_installments, payment_value)
  ΓÇó olist_order_reviews_dataset (review_id, order_id, review_score, review_comment_title, review_comment_message)
  ΓÇó olist_geolocation_dataset (geolocation_zip_code_prefix, geolocation_lat, geolocation_lng, geolocation_city, geolocation_state)
  ΓÇó olist_product_category_name_translation (product_category_name, product_category_name_english)
...
Your responsibilities:
  ...
  3. Perform **data quality analysis** ΓÇö identify nulls, duplicates, anomalies.
...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# TABS  ΓåÉ added Visual Analytics tab
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
tab1, tab2, tab3, tab4 = st.tabs(
    ["≡ƒÆ¼ Data Steward Chat", "≡ƒôè Schema Map & Quality", "≡ƒôê Column Profiler", "≡ƒôè Visual Analytics"]
)
...
with tab1:
    ...
    suggestions = [
        "≡ƒôï Describe all tables",
        "≡ƒöù Show table relationships",
        "≡ƒ¢á∩╕Å Write a JOIN query",
        "≡ƒöì Find anomalies",
        "≡ƒôè Suggest KPIs",
    ]
    ...
    # Chat input
    if prompt := st.chat_input("Ask about the data schema ΓÇª"):
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                with st.status("≡ƒºá SchemaSense is thinking ΓÇª", expanded=False):
                    ...
                    for chunk in responses:
                        if chunk.text:
                            ...
                            response_placeholder.markdown(full_response + "Γûî")
                ...
                if sql:
                    ...
                    st.info("≡ƒÆí SQL detected! Switch to the **≡ƒôè Visual Analytics** tab to run it.")

            except Exception as e:
                err_msg = str(e)
                if "quota" in err_msg.lower() or "429" in err_msg:
                    st.error(
                        "ΓÜá∩╕Å **All API quota keys exhausted.** Please wait a moment and try again, "
...
with tab2:
    ...
    with col_left:
        st.markdown(
            ...
            "<h3 style='margin-top:0'>≡ƒ¢í∩╕Å Intelligent Quality Guard</h3>"
        ...
        if st.button("≡ƒÜÇ Run Data Quality Scan", type="primary", use_container_width=True):
            with st.status("≡ƒö¼ Analyzing schema health ΓÇª", expanded=True) as status:
                quality_prompt = (
                    "Execute the Intelligent Quality Guard protocol. "
                    "Generate a comprehensive Data Quality Report for the active dataset. "
                    "Include sections for: "
                    "1) **Overview** ΓÇô dataset summary stats. "
                    "2) **Completeness** ΓÇô missing/null analysis per column. "
                    "3) **Consistency** ΓÇô data type mismatches or formatting issues. "
                    "4) **Anomalies** ΓÇô outlier risks and statistical health. "
                    "5) **Recommendations** ΓÇô prioritized actions to improve data quality. "
                    "Use tables, bullet points, and severity emojis (≡ƒƒó≡ƒƒí≡ƒö┤)."
                )
                # FEATURE 1: key rotation wrapper
                response, key_idx = generate_with_fallback(
                    selected_model, steward_persona, quality_prompt
                )
                status.update(label="Γ£à Scan complete!", state="complete")
    ...
    with col_right:
        if data_source == "Built-in: Olist E-Commerce":
            st.markdown(
                '<div class="glass-card">'
                "<h3 style='margin-top:0'>≡ƒù║∩╕Å Olist Relationship Map</h3>"
                "</div>",
                unsafe_allow_html=True,
            )
    ...
    CATEGORY_TRANSLATION ||--|| PRODUCTS : translates'''
            ...
            if st.session_state.uploaded_meta:
                st.markdown(
                    '<div class="glass-card">'
                    "<h3 style='margin-top:0'>≡ƒôï Schema Summary</h3>"
                    "</div>",
                    unsafe_allow_html=True,
                )
                ...
                if len(st.session_state.sqlite_schema) > 1:
                    st.markdown("**≡ƒôª Loaded Tables:**")
                    for tname, cols in st.session_state.sqlite_schema.items():
                        st.markdown(f"- `{tname}`: {', '.join([f'`{c}`' for c in cols[:6]])}{'ΓÇª' if len(cols) > 6 else ''}")
            else:
                st.info("≡ƒôé Upload a dataset to generate insights.")


# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
# TAB 3 ΓÇô COLUMN PROFILER
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
with tab3:
    st.markdown(
        '<div class="glass-card">'
        "<h3 style='margin-top:0'>≡ƒôê Column Profiler</h3>"
    ...
    if data_source == "Upload Custom Dataset" and st.session_state.uploaded_df is not None:
        ...
        profile_tab_num, profile_tab_cat = st.tabs(["≡ƒöó Numeric Columns", "≡ƒÅ╖∩╕Å Categorical Columns"])
        ...
        with profile_tab_num:
            if numeric_cols:
                ...
                for col in sel_num:
                    with st.expander(f"≡ƒôè {col}", expanded=True):
                        c1, c2 = st.columns(2)
                        with c1:
                            fig = px.histogram(
                                df, x=col, nbins=30,
        ...
        with profile_tab_cat:
            if categorical_cols:
                ...
                for col in sel_cat:
                    with st.expander(f"≡ƒÅ╖∩╕Å {col}", expanded=True):
                        value_counts = df[col].value_counts().head(15)
                        fig = px.bar(
                            x=value_counts.index.astype(str),
                            y=value_counts.values,
                            title=f"Top Values ΓÇô {col}",
        ...
        st.info("≡ƒôé Upload a CSV in the sidebar to start profiling columns.")


# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
# TAB 4 ΓÇô VISUAL ANALYTICS ENGINE  (FEATURE 3)
# Dual-mode: PostgreSQL (Olist) or SQLite (CSV uploads)
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
with tab4:
    # ΓöÇΓöÇ Determine active backend ΓöÇΓöÇ
    _using_postgres = (data_source == "Built-in: Olist E-Commerce")
    _sql_hint = "PostgreSQL" if _using_postgres else "SQLite"

    st.title("≡ƒôè Visual Analytics Engine")
    st.write(f"Execute SQL against the **{_sql_hint}** database. Queries are auto-detected from the chat.")

    # ΓöÇΓöÇ Branch: Olist PostgreSQL vs CSV SQLite ΓöÇΓöÇ
    if _using_postgres:
        # ΓöÇΓöÇ PostgreSQL path ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        if pg_conn is None:
            st.error(
                "≡ƒö┤ **PostgreSQL connection unavailable.** "
            ...
            with st.expander('≡ƒôè Data Schema Details', expanded=False):
                st.markdown('''
* **olist_orders_dataset** ΓÇö order_id, customer_id, order_status, order_purchase_timestamp...
* **olist_order_items_dataset** ΓÇö order_id, product_id, seller_id, price, freight_value...
* **olist_products_dataset** ΓÇö product_id, product_category_name...
* **olist_customers_dataset** ΓÇö customer_id, customer_city, customer_state...
* **olist_sellers_dataset** ΓÇö seller_id, seller_city, seller_state...
* **olist_order_payments_dataset** ΓÇö order_id, payment_type, payment_value...
* **olist_order_reviews_dataset** ΓÇö review_id, order_id, review_score...
* **olist_geolocation_dataset** ΓÇö geolocation_zip_code_prefix, geolocation_lat, geolocation_lng...
* **olist_product_category_name_translation** ΓÇö product_category_name, product_category_name_english...
''')
            ...
            if default_sql:
                st.success("Γ£à SQL auto-detected from the latest AI response. You can edit it below.")

            sql_input = st.text_area(
                "≡ƒô¥ SQL Query (PostgreSQL syntax):",
                value=default_sql,
                height=160,
                placeholder=(
            ...
            with col_run:
                run_btn = st.button("Γû╢∩╕Å Run Query", type="primary", use_container_width=True, key="pg_run")
            with col_clear:
                if st.button("≡ƒùæ∩╕Å Clear SQL", use_container_width=True, key="pg_clear"):
                    st.session_state.last_sql = None
                    st.rerun()

            if run_btn and sql_input.strip():
                try:
                    with st.spinner("ΓÜí Querying PostgreSQL (Cloud SQL)ΓÇª"):
                        # conn.query() returns a DataFrame and caches results for 10 minutes
                        result_df = pg_conn.query(sql_input.strip(), ttl="10m")

                    st.success(
                        f"Γ£à Query returned **{len(result_df):,} rows** ├ù "
                    ...
                    # ΓöÇΓöÇ Auto-Chart ΓöÇΓöÇ
                    if result_df.empty:
                        st.warning("Query returned 0 rows ΓÇö no chart to display.")
                    else:
                        ...
                        if numeric_res_cols:
                            st.markdown("### ≡ƒôê Auto-Generated Bar Chart")
                            ...
                            if non_numeric_res_cols and numeric_res_cols:
                                with st.expander("≡ƒÄ¿ Enhanced Plotly Chart", expanded=False):
                                    fig = px.bar(
                                        result_df.head(30),
                            ...
                            st.info("Γä╣∩╕Å No numeric columns in the result ΓÇô displaying table only.")

                except Exception as e:
                    st.error(f"Γ¥î PostgreSQL Error: {e}")
                    st.caption(
                ...
                st.warning("ΓÜá∩╕Å Please enter a SQL query first.")

    else:
        # ΓöÇΓöÇ SQLite / CSV upload path ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        sqlite_conn = st.session_state.sqlite_conn

        if sqlite_conn is None:
            st.info(
                "≡ƒôé **No database loaded.** Please upload one or more CSV files using "
                "'Upload Custom Dataset' in the sidebar to enable the Visual Analytics Engine."
            )
        else:
            schema = st.session_state.sqlite_schema
            if schema:
                with st.expander("≡ƒôï Available Tables & Columns", expanded=False):
                    for tname, cols in schema.items():
                        st.markdown(f"**`{tname}`** ΓÇö {', '.join([f'`{c}`' for c in cols])}")
            ...
            col_run, col_clear = st.columns([1, 4])
            with col_run:
                run_btn = st.button("Γû╢∩╕Å Run Query", type="primary", use_container_width=True, key="sqlite_run")
            with col_clear:
                if st.button("≡ƒùæ∩╕Å Clear SQL", use_container_width=True, key="sqlite_clear"):
                    st.session_state.last_sql = None
                    st.rerun()

            if run_btn and sql_input.strip():
                try:
                    with st.spinner("ΓÜí Executing queryΓÇª"):
                        result_df = pd.read_sql(sql_input.strip(), sqlite_conn)

                    st.success(f"Γ£à Query returned **{len(result_df):,} rows** ├ù **{len(result_df.columns)} columns**")
                    st.dataframe(result_df, use_container_width=True)
                    ...
                    if result_df.empty:
                        ...
                        if numeric_res_cols:
                            ...
                            st.info("Γä╣∩╕Å No numeric columns in the result ΓÇô displaying table only (no chart).")

                except Exception as e:
                    st.error(f"Γ¥î SQL Error: {e}")
...
st.markdown(
    ...
    "Built with Γ¥ñ∩╕Å for GDG Cloud Delhi Hackathon ┬╖ Powered by Google Gemini & Streamlit"
    "</p>",
    unsafe_allow_html=True,
)
